...
import { Package, Search, Calendar, CheckCircle2, AlertTriangle, Clock, History, User, CheckCircle } from 'lucide-react';
...
const Packages: React.FC = () => {
  ...
  const activePackages = useMemo(() => {
    return visiblePackages.filter(pkg => {
      ...
      // Exibe se ainda tiver sessões para consumir (Realizadas/Faltas < Total)
  ...
  const finishedPackages = useMemo(() => {
    return visiblePackages.filter(pkg => {
      const patient = visiblePatients.find(p => p.id === pkg.patientId);
      const matchesSearch = patient?.name.toLowerCase().includes(searchTerm.toLowerCase());
      // Exibe apenas se as 4 sessões foram efetivamente consumidas
      return pkg.usedSessions >= pkg.totalSessions && matchesSearch;
    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500">Apenas sessões realizadas ou faltas sem aviso deduzem saldo.</p>
        </div>
      ...
      <section className="space-y-6">
        <h3 className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em] flex items-center gap-2">
          <Clock size={14} /> Pacotes em Evolução
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activePackages.length > 0 ? (
                  ...
                  <div className="p-8 space-y-5">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-gray-300 font-bold uppercase tracking-wider">ID: {pkg.id.slice(0, 8)}</span>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs font-black uppercase tracking-widest">
                        <span className="text-gray-400">Dedução (Uso Real)</span>
                      ...
                      {details.scheduledCount > 0 && (
                        <p className="text-[9px] font-bold text-amber-500 flex items-center gap-1"><AlertTriangle size={10}/> {details.scheduledCount} sessões agendadas (não deduzidas)</p>
                      )}
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-gray-50/80 p-4 rounded-2xl border border-gray-100 text-center">
                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Restantes</p>
                        <p className="text-2xl font-black text-indigo-600">{pkg.remainingSessions}</p>
                      </div>
                      <div className="bg-gray-50/80 p-4 rounded-2xl border border-gray-100 text-center flex flex-col justify-center">
                        <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Validade</p>
                    ...
                    {details.dates && (
                      <div className="pt-4 border-t border-gray-50">
                        <p className="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-1">Consumido em:</p>
            ...
            <div className="col-span-full py-16 bg-gray-50/50 rounded-[32px] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-400">
              ...
              <p className="font-bold text-sm">Nenhum pacote com saldo ativo.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="space-y-6">
        <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] flex items-center gap-2">
          <CheckCircle size={14} /> Pacotes Concluídos (4/4 Sessões)
        </h3>
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50/50 text-[10px] font-black text-gray-400 uppercase tracking-widest">
              <tr>
                ...
                <th className="px-8 py-5">Uso Efetivo (Datas)</th>
              ...
              {finishedPackages.length > 0 ? finishedPackages.map((pkg) => {
                ...
                return (
                  <tr key={pkg.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-8 py-5">
                      <p className="text-xs text-gray-500 leading-relaxed max-w-md line-clamp-1 italic">
                        {details.dates || 'Sem datas registradas'}
                      </p>
                    </td>
                ...
                <tr>
                  <td colSpan={4} className="px-8 py-12 text-center text-gray-400 italic text-sm">
                    Nenhum pacote finalizado (4/4) encontrado.
                  </td>
                </tr>
              )}
            </tbody>
